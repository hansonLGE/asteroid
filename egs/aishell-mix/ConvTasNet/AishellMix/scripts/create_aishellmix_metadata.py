...
parser.add_argument('--aishell_md_dir', type=str, required=True,
                    help='Path to aishell metadata directory')
...
parser.add_argument('--metadata_outdir', type=str, default=None,
                    help='Where aishellmix metadata files will be stored.')
...
def main(args):
    ...
    aishell_md_dir = args.aishell_md_dir
    wham_dir = args.wham_dir
    wham_md_dir = args.wham_md_dir
    n_src = args.n_src
    # Create Aishellmix metadata directory
    md_dir = args.metadata_outdir
    if md_dir is None:
        root = os.path.dirname(aishell_dir)
        md_dir = os.path.join(root, f'AishellMix/metadata')
    os.makedirs(md_dir, exist_ok=True)
    create_aishellmix_metadata(aishell_dir, aishell_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src)


def create_aishellmix_metadata(aishell_dir, aishell_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src):
    """ Generate AishellMix metadata according to Aishell metadata """

    # Dataset name
    dataset = f'aishell{n_src}mix'
    # List metadata files in Aishell
    aishell_md_files = os.listdir(aishell_md_dir)
    ...
    for aishell_md_file in aishell_md_files:
        if not aishell_md_file.endswith('.csv'):
            print(f"{aishell_md_file} is not a csv file, continue.")
            continue
        # Get the name of the corresponding noise md file
        try:
            wham_md_file = [f for f in wham_md_files if
                            f.startswith(aishell_md_file)][0]
        except IndexError:
            print('Wham metadata are missing you can either generate the '
                  'missing wham files or add the aishell metadata to '
                  'to_be_ignored list')
            break

        # Open .csv files from Aishell
        aishell_md = pd.read_csv(os.path.join(
            aishell_md_dir, aishell_md_file), engine='python')
        ...
        save_path = os.path.join(md_dir,
                                 '_'.join([dataset, aishell_md_file]))
        info_name = '_'.join([dataset, os.path.splitext(aishell_md_file)[0],
                              'info']) + '.csv'
        info_save_path = os.path.join(md_dir, info_name)
        print(f"Creating {os.path.basename(save_path)} file in {md_dir}")
        # Create dataframe
        mixtures_md, mixtures_info = create_aishellmix_df(
            aishell_md, aishell_dir, wham_md, wham_dir,
            n_src)
...
def check_already_generated(md_dir, dataset, to_be_ignored,
    ...
    # Check if the metadata files in Aishell already have been used
    ...
    for element in to_be_ignored:
        aishell_md_files.remove(element)


def create_aishellmix_df(aishell_md_file, aishell_dir,
                       wham_md_file, wham_dir, n_src):
    """ Generate aishellmix dataframe from a Aishell and wha md file"""
    ...
    pairs, pairs_noise = set_pairs(aishell_md_file, wham_md_file, n_src)
    clip_counter = 0
    ...
    for pair, pair_noise in tqdm(zip(pairs, pairs_noise), total=len(pairs)):
        ...
        sources_info, sources_list_max = read_sources(
            aishell_md_file, pair, n_src, aishell_dir)
        # Add noise
...
def set_pairs(aishell_md_file, wham_md_file, n_src):
    """ set pairs of sources to make the mixture """
    # Initialize list for pairs sources
    utt_pairs = []
    noise_pairs = []
    # In train sets utterance are only used once
    if 'train' in aishell_md_file.iloc[0]['subset']:
        utt_pairs = set_utt_pairs(aishell_md_file, utt_pairs, n_src)
        noise_pairs = set_noise_pairs(utt_pairs, noise_pairs,
                                      aishell_md_file, wham_md_file)
    # In dev sets utterance are only used once, 5000 mixtures
    elif 'dev' in aishell_md_file.iloc[0]['subset']:
        utt_pairs = set_utt_pairs(aishell_md_file, utt_pairs, n_src)
...
def set_utt_pairs(aishell_md_file, pair_list, n_src):
    # A counter
    c = 0
    # Index of the rows in the metadata file
    index = list(range(len(aishell_md_file)))
    ...
    while len(index) >= n_src and c < 200:
        ...
        speaker_list = set([aishell_md_file.iloc[couple[i]]['speaker_ID']
                            for i in range(n_src)])
...
def set_noise_pairs(pairs, noise_pairs, aishell_md_file, wham_md_file):
    ...
    for pair in pairs.copy():
        # get sources infos
        sources = [aishell_md_file.iloc[pair[i]]
...
def read_sources(metadata_file, pair, n_src, aishell_dir):
    ...
    for i in range(n_src):
        ...
        absolute_path = os.path.join(aishell_dir,
                                     source['origin_path'])
...
def set_loudness(sources_list):
    ...
    # Initialize loudness
    loudness_list = []
    # In Aishell all sources are at 16KHz hence the meter
