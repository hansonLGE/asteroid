...
EPS = 1e-10
# Rate of the sources in Aishell
RATE = 16000
...
parser.add_argument('--metadata_dir', type=str, required=True,
                    help='Path to the AishellMix metadata directory')
parser.add_argument('--aishellmix_outdir', type=str, default=None,
...
def main(args):
    # Get aishell root path
    aishell_dir = args.aishell_dir
    wham_dir = args.wham_dir
    # Get Metadata directory
    metadata_dir = args.metadata_dir
    # Get AishellMix root path
    aishellmix_outdir = args.aishellmix_outdir
    n_src = args.n_src
    if aishellmix_outdir is None:
        aishellmix_outdir = os.path.dirname(metadata_dir)
    aishellmix_outdir = os.path.join(aishellmix_outdir, f'Aishell{n_src}Mix')
    ...
    create_aishellmix(aishell_dir, wham_dir, aishellmix_outdir, metadata_dir,
                    freqs, n_src, modes, types)


def create_aishellmix(aishell_dir, wham_dir, out_dir, metadata_dir,
                    freqs, n_src, modes, types):
    """ Generate sources mixtures and saves them in out_dir"""
    # Get metadata files
    md_filename_list = [file for file in os.listdir(metadata_dir)
                        if 'info' not in file]
    # Create all parts of aishellmix
    for md_filename in md_filename_list:
        csv_path = os.path.join(metadata_dir, md_filename)
        process_metadata_file(csv_path, freqs, n_src, aishell_dir,
                              wham_dir, out_dir, modes, types)


def process_metadata_file(csv_path, freqs, n_src, aishell_dir, wham_dir,
                          out_dir, modes, types):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            process_utterances(md_file, aishell_dir, wham_dir, freq, mode,
                               subdirs, dir_path, subset_metadata_path, n_src)


def process_utterances(md_file, aishell_dir, wham_dir, freq, mode, subdirs,
    ...
    for results in tqdm.contrib.concurrent.process_map(
        functools.partial(
            process_utterance, 
            n_src, aishell_dir, wham_dir, freq, mode, subdirs, dir_path),
...
def process_utterance(n_src, aishell_dir, wham_dir, freq, mode, subdirs, dir_path, row):
    res = []
    # Get sources and mixture infos
    mix_id, gain_list, sources = read_sources(row, n_src, aishell_dir,
                                              wham_dir)
    # Transform sources
...
def read_sources(row, n_src, aishell_dir, wham_dir):
    ...
    for sources_path in sources_path_list:
        sources_path = os.path.join(aishell_dir,
                                    sources_path)
...
def transform_sources(sources_list, freq, mode, gain_list):
    """ Transform aishell sources to aishellmix """
    # Normalize sources
